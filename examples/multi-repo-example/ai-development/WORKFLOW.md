...
- **Design-driven changes.** *Direct path* (recommended): the human commits the design tool's export under `design/<tool>/`, and the agent implements it in the project's stack with real data, classified by a requirements table. *Builder path* (optional): an app-builder writes a runnable app with fake data to a design source branch that reaches production only through a pull request; the presentation is kept, and fake boundaries are found by behavior and listed in an extraction table: [protocol/DESIGN-DRIVEN.md](protocol/DESIGN-DRIVEN.md).
