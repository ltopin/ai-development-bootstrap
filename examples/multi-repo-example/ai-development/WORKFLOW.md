...
- **Design-driven changes.** Implement the design reference under `design/<tool>/` in the project's stack with real data, classified by a requirements table. Naming the design in the tool exports it; commit the export alone before implementing; push only when the human asks: [protocol/DESIGN-DRIVEN.md](protocol/DESIGN-DRIVEN.md).
