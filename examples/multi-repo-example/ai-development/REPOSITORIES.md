...
|---|---|---|---|---|---|
| Orders HTTP API | `api` | `web`, third parties | HTTP / OpenAPI | `../api/openapi.yaml` (illustrative path) | confirmed |
| `order.status_changed` event | `api` | `worker` | Async event | [docs/domains/orders.md](docs/domains/orders.md) | confirmed |
| Orders tables | `api` | `worker` (read-only) | Database | `../api/migrations/` (illustrative path) | confirmed |
...
- `worker` → `email-provider`: found only as an API key variable in config; provider and delivery guarantees not confirmed.
